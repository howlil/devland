...
test('localized reversible changes stay rapid with minimal execution budgets', async () => {
...
test('guided change signals select targeted analysis and affected verification', async () => {
...
test('security boundary changes are deliberate and activate reusable security guidance', async () => {
  ...
  assert.deepEqual(context.execution.budget, {
    analysis: 'deliberate',
...
test('the highest material risk wins when signals are mixed', () => {
  ...
  assert.equal(result.budget.analysis, 'deliberate');
  assert.equal(result.budget.verification, 'strong');
});
...
test('context CLI accepts transient change metadata without persisting it to canonical state', () => {
  ...
  assert.equal(output.execution.budget.context, 'risk-expanded');
